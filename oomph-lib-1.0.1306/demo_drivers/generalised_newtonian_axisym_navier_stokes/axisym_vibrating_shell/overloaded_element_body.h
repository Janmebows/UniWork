...
//LIC//====================================================================
 ...
 public:
    ...
    for (unsigned val=0;val<fp_n_val;val++)
     {
      ...
      for (unsigned ipt=0;ipt<n_intpt;ipt++)
       {
        Fixed_point_iteration_guess_for_strain_rate[val][ipt].resize(3,3,0.0);
       }
     }
  ...
  void update_latest_fixed_point_iteration_guess_for_strain_rate()
   {
    Vector<double> s(2);
    DenseMatrix<double> strain_rate(3,3,0.0);
    unsigned n_intpt = integral_pt()->nweight();
    for (unsigned ipt=0;ipt<n_intpt;ipt++)
     {
  ...
  void extrapolated_strain_rate(const Vector<double>& s, 
                                DenseMatrix<double>& strainrate) const
   {
    ...
    for (unsigned it=0;it<Nprev_for_extrapolation_of_strain_rate;it++)
     {
      previous_strain_rate[it].resize(3,3);
  ...
  std::string variable_identifier()
   {
    ...
    //txt+="\"strain_rate_m1(2,2)\","; //33
    //txt+="\"strain_rate_m1(0,1)\","; //34
    //txt+="\"strain_rate_m2(0,0)\","; //v35
    //txt+="\"strain_rate_m2(1,1)\","; //v36
    //txt+="\"strain_rate_m2(2,2)\","; //v37
    //txt+="\"strain_rate_m2(0,1)\","; //v38
    //txt+="\"strain_rate_extrapol(0,0)\","; //v39
    //txt+="\"strain_rate_extrapol(1,1)\","; //v40
    //txt+="\"strain_rate_extrapol(2,2)\","; //v41
    //txt+="\"strain_rate_extrapol(0,1)\","; //v42
    ...
    for (unsigned iplot=0;iplot<num_plot_points;iplot++)
     {
      ...
      DenseMatrix<double> interpolated_dudx(el_dim+1,el_dim,0.0);
      ...
      for(unsigned i=0;i<el_dim;i++)
       {
        ...
        for(unsigned l=0;l<n_node;l++) 
         {
          dudt[i]+=du_dt_axi_nst(l,u_nodal_index)*psif[l];
      ...
      // Stress
      DenseMatrix<double> stress(3,3,0.0);
      ...
      //outfile << rate_of_strain_extrapol(2,2) << " ";
    ...
    for(unsigned ipt=0;ipt<n_intpt;ipt++)
     {
      ...
      // Invariant of actual strain rate tensor
      DenseMatrix<double> current_strainrate(3,3,0.0);
      ...
      // Invariant of extrapolated strain rate tensor
      // (or recycle  actual one if there's no extrapol)
      DenseMatrix<double> latest_guess_strainrate(3,3,0.0);
  ...
  double square_of_l2_norm()
   {
    ...
    for(unsigned i=0;i<el_dim+1;i++) {u_nodal_index[i] = u_index_axi_nst(i);}
    ...
    for(unsigned ipt=0;ipt<n_intpt;ipt++)
     {
      ...
      Vector<double> interpolated_u(el_dim+1,0.0);
