...
//LIC//====================================================================
//------------------------------------------------------------------------
// Flow past cylinder -- developed by Stefan Kollmannsberger
// and students Iason Papaioannou and Orkun Oezkan Doenmez
//
...
public:
 ...
 {
  for (unsigned i=0;i<8;i++){delete Macro_element_pt[i];}
 }
...
{ 
 ...
 for(unsigned ibound=0;ibound<num_bound;ibound++)
  ...
  //Fix 0-th pressure value in last element to 0.0.
  dynamic_cast<ELEMENT*>(mesh_pt()->element_pt((mesh_pt()->nelement())-1))->
   fix_pressure(0,0.0);
  ...
  for (unsigned e=0;e<nelem;e++)
   {
    dynamic_cast<ELEMENT*>(mesh_pt()->element_pt(e))->re_pt()=
  ...
  // Set parabolic axial velocity at inlet. This is automatically
  // interpolated onto the newly created nodes when the mesh is
  // adapted. Since the velocity is not constant, the interpolation is insufficiently
  // accurate and the boundary conditions must be re-applied
  // in actions_after_adapt().
  ...
  for (unsigned inod=0;inod<num_nod;inod++)
   {
    ycoord = mesh_pt()->boundary_node_pt(ibound,inod)->x(1);
    uy = 1./0.2*4.0*um*ycoord*(height-ycoord)/height/height;
    // Tangential flow
    mesh_pt()->boundary_node_pt(ibound,inod)->set_value(0,uy);
    // No penetration
