...
//LIC// 
//LIC//====================================================================
//Driver for a 2D linearised shell problem: circular tube
...
//Header file for 2d linear shell problem: circular-tube bending
...
public:
 ...
 virtual void output_fct(std::ostream &outfile, const unsigned &n_plot,
  ...
  {
   throw OomphLibError(
    "There is no time-dependent output_fct() for shell elements ",
 ...
 /// \short Return FE representation of unknown value u(s) 
 ...
 inline Vector<double> interpolated_u_shell(const Vector<double> &s) const
  {
   ...
   //Get values of c0-shape function
   this->basis_c0(s,psi);
   ...
   //Get values of c1-shape function
 ...
 for(unsigned ipt=0;ipt<n_intpt;ipt++)
  {
   ...
   //Calculate displacement values and derivatives:
   //-----------------------------------------
   //in both tangential directions
   for(unsigned i=0;i<2;i++) 
    {
     //Loop over c0-test functions
     for(unsigned l=0;l<n_node;l++)
      {
   ...
   DenseMatrix<double> t(3,n_dim,0.0),t_tn(3,n_dim,0.0);
   ...
   // decompose in the normal and tangential coordinates
   ...
   for(unsigned i=0;i<n_dim;i++)
    {
     ...
     else /// find residuals for the first and second tangential directions
      {
...
{ 
 ...
 Undef_midplane_pt = new EllipticalTube(1.0,1.0);
 ...
 //------- CIRCULAR TUBE BENDING PROBLEM -----------
 // Pin the nodal values at nodes on mesh 
 // boundary 0 (= the lower domain boundary at y=0)
 ...
 // boundary 2 (= the right domain boundary at y=1)
 ...
 // boundary 3 (= the right domain boundary at x=0)
 for(unsigned i=0;i<n_side3;i++)
  {
   mesh_pt()->boundary_node_pt(3,i)->pin(0); 
   mesh_pt()->boundary_node_pt(3,i)->pin(1); 
   mesh_pt()->boundary_node_pt(3,i)->pin(2); 
   mesh_pt()->boundary_node_pt(3,i)->pin(3); 
   mesh_pt()->boundary_node_pt(3,i)->pin(4);  
   mesh_pt()->boundary_node_pt(3,i)->pin(5); 
   mesh_pt()->boundary_node_pt(3,i)->pin(6); 
   mesh_pt()->boundary_node_pt(3,i)->pin(7); 
...
{ 
 ...
 // Set up doc info
 DocInfo doc_info;
 doc_info.set_directory("RESLT_unstructured_curved_shell");
